...
def strip_unit_prefix(unit_name: str) -> str:
    """Strip 'Unit X —' or 'Unit X -' prefix from unit name."""
    if not unit_name:
        return unit_name
    # Match patterns like "Unit 4 — ", "Unit 4 - ", "Unit 4: ", "Unit 4. " at the start
    # Also handles em dash (—), en dash (–), and regular hyphen (-)
    cleaned = re.sub(r'^Unit\s+\d+\s*[—–\-:\.]\s*', '', unit_name, flags=re.IGNORECASE)
        ...
        try:
            ...
            for line in lines:
                ...
                if line.startswith('UNIT:'):
                    ...
                    if unit_name:
                        # Strip "Unit X —" prefix and normalize dashes
